...
- 🔔 **Notifications**: **Pushover**, **Discord webhooks** and **browser notifications** — with a
  configurable cooldown so you don't get spammed. Each alert is detailed: type/operator, registration,
  squawk, altitude (with climb/descent), speed, **heading + compass**, route when known, distance, the
  aircraft's **estimated location** (nearest village/town/city, e.g. *📍 near Aarschot, Belgium*,
  reverse-geocoded via OpenStreetMap Nominatim — free, no key), its **coordinates** and a **map link**
...
- 🛬 **Arrivals board** (own tab) — a live airport-style FIDS: every inbound flight sorted by ETA with
  ...
  find it on the map
- 🛰 **Anomaly detection** — detectors that run on the live ADS-B stream (no extra data
  ...
  **emergency descent**, **ADS-B integrity/spoofing** checks (impossible kinematics, reserved ICAO
  blocks, NIC collapse, speed/position mismatch), **rarity scoring** (first-time type/operator in
  your coverage — permanent, all-time memory), **survey/patrol-grid** detection (parallel-leg "mowing the lawn"),
  **go-around / missed-approach** detection, and a **military fleet feed** (cross-references adsb.lol/
  adsb.fi's military list). Surfaced in a dedicated **Detections** tab and the normal alert pipeline
